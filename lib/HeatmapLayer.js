...
var _lodash13 = require('lodash.isnumber');
...
function isInvalid(num) {
  return !(0, _lodash14.default)(num) && !num;
}
...
function computeAggregate(agg, intensity) {
  var aggregateType = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'sum';

  /* eslint-disable no-use-before-define */
  var updateMeanAndSigma = function updateMeanAndSigma(m, c, v) {
  ...
  var fns = {
    mean: function mean(m, c, v) {
      return (v - m) / c;
    },
    count: function count(m, c, v) {
      return 1;
    },
    sum: function sum(m, c, v) {
      return v;
    },
    distinct: function distinct(m, c, v) {
      agg.same.push(v);
      return (0, _lodash12.default)(agg.same).length;
    },
    min: function min(m, c, v) {
      return Math.min(m, v);
    },
    max: function max(m, c, v) {
      return Math.max(m, v);
    },
    variance: function variance(m, c, v) {
      updateMeanAndSigma(m, c, v);
      return c > 1 ? agg.data.sigma / (c - 1) : 0;
    },
    variancep: function variancep(m, c, v) {
      updateMeanAndSigma(m, c, v);
      return c > 1 ? agg.data.sigma / c : 0;
    },
    stdev: function stdev(m, c, v) {
      return Math.sqrt(fns.variance(m, c, v));
    },
    stdevp: function stdevp(m, c, v) {
      return Math.sqrt(fns.variancep(m, c, v));
    }
  };
...
exports.default = (0, _reactLeaflet.withLeaflet)((_temp = _class = function (_MapLayer) {
  ...
  HeatmapLayer.prototype.getMax = function getMax(props) {
    return (0, _lodash14.default)(props.max) ? props.max : 3.0;
  };

  HeatmapLayer.prototype.getRadius = function getRadius(props) {
    return (0, _lodash14.default)(props.radius) ? props.radius : 30;
  };

  HeatmapLayer.prototype.getMaxZoom = function getMaxZoom(props) {
    return (0, _lodash14.default)(props.maxZoom) ? props.maxZoom : 18;
  };

  HeatmapLayer.prototype.getMinOpacity = function getMinOpacity(props) {
    return (0, _lodash14.default)(props.minOpacity) ? props.minOpacity : 0.01;
  };

  HeatmapLayer.prototype.getBlur = function getBlur(props) {
    return (0, _lodash14.default)(props.blur) ? props.blur : 15;
  };
  ...
  HeatmapLayer.prototype.updateHeatmapMax = function updateHeatmapMax(maximum) {
    if ((0, _lodash14.default)(maximum)) {
  ...
  HeatmapLayer.prototype.redraw = function redraw() {
    ...
    var roundResults = function roundResults(results) {
      return (0, _lodash4.default)(results, function (result, row) {
        return (0, _lodash2.default)(filterUndefined(row), function (cell) {
          return [Math.round(cell[0]), Math.round(cell[1]), cell[2]];
        }).concat(result);
      }, []);
    };
    ...
    var accumulateInGrid = function accumulateInGrid(points, leafletMap, bounds, aggregateType) {
    ...
    var getDataForHeatmap = function getDataForHeatmap(points, leafletMap, aggregateType) {
      return roundResults(accumulateInGrid(points, leafletMap, getBounds(leafletMap), aggregateType));
    };

    var data = getDataForHeatmap(this.props.points, this.props.leaflet.map, this.props.aggregateType);

    var totalMax = (0, _lodash10.default)(data.map(function (m) {
      return m[2];
    }));
  ...
  aggregateType: _propTypes2.default.oneOf(['mean', 'count', 'sum', 'distinct', 'min', 'max', 'variance', 'variancep', 'stdev', 'stdevp'])
}, _temp));
